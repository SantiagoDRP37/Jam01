...
public class PlayerController_beta : MonoBehaviour
{
    ...
    {
        // Verifica si el objeto con el que colision� es el enemigo (skeleton)
        if ((collision.gameObject.CompareTag("Skeleton") || collision.gameObject.CompareTag("Ghost")) && !PolygonCollider2D.enabled)
        {
            // Acci�n cuando el enemigo toca al personaje
            Animator.SetTrigger("Hurt");
            uIController.UpdateHealthDisplay();

            // Aqu� puedes a�adir lo que deber�a ocurrir: perder vida, cambiar de animaci�n, etc.
        }

        // Verificca si colisiona con el suelo da�ino
        if (collision.gameObject.CompareTag("Pain"))
        {
            Animator.SetTrigger("Hurt");
        }

        //Detecta colisi�n con el final
        if (collision.gameObject.CompareTag("FinalB"))
        {
            Time.timeScale = 0f; //esta condici�n se debe cambiar por la accion final
        }
    }
